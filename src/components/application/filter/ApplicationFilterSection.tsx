...
export const ApplicationFilterSection = ({
  ...
  const [openAccordion, setOpenAccordion] = useState<'' | 'status' | 'sort'>('')
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full gap-3 sm:w-auto sm:gap-4 lg:gap-6">
          <StatusSelect
            ...
            open={openAccordion === 'status'}
            setOpen={(v) => setOpenAccordion(v ? 'status' : '')}
          />
          <SortSelect
            value={sortKey}
            onChange={setSortKey}
            open={openAccordion === 'sort'}
            setOpen={(v) => setOpenAccordion(v ? 'sort' : '')}
          />
        </div>
      </div>
    </section>
